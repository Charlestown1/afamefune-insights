...
const tradeSchema = new mongoose.Schema({
  ...
  aiAnalysisText: { type: String, default: null }, // raw text fallback; always populated when Gemini succeeds
  aiVerdict: { type: String, default: null }, // 'Strong Setup' | 'Good Setup' | 'Neutral Setup' | 'Weak Setup' | 'Avoid'
  aiScores: {
    technical: { type: Number, default: null },
    risk: { type: Number, default: null },
    execution: { type: Number, default: null },
    psychology: { type: Number, default: null },
    overall: { type: Number, default: null }
  },
  aiStructured: { type: mongoose.Schema.Types.Mixed, default: null }, // full structured breakdown, Phase 2
  // Phase 3: optional psychology check-in, all fields default null/undefined
  // so old trades and trades where the user skips this remain fully valid.
  psychology: {
    confidence: { type: Number, default: null }, // 1-10, pre-trade
    fear: { type: Number, default: null },
    fomo: { type: Number, default: null },
    patience: { type: Number, default: null },
    revengeUrge: { type: Number, default: null },
    followedPlan: { type: Boolean, default: null }, // post-trade
    movedStopLoss: { type: Boolean, default: null },
    closedEarly: { type: Boolean, default: null },
    revengeTraded: { type: Boolean, default: null },
    overLeveraged: { type: Boolean, default: null },
    followedStrategy: { type: Boolean, default: null }
  },
  ...
  realizedR: { type: Number, default: null },    // R-multiple actually achieved, derived from entry/stopLoss/exit
  createdAt: { type: Date, default: Date.now },
...
// Phase 3: safely parses the optional psychology check-in object from the
// request body. Every field is independently optional — a partially filled
// form, a missing object entirely, or garbage input all resolve to nulls
// rather than throwing or silently corrupting the rest of the trade save.
function parsePsychology(raw) {
  ...
  return {
    confidence: clampRating(raw.confidence),
    fear: clampRating(raw.fear),
    fomo: clampRating(raw.fomo),
    patience: clampRating(raw.patience),
    revengeUrge: clampRating(raw.revengeUrge),
    followedPlan: toBoolOrNull(raw.followedPlan),
    movedStopLoss: toBoolOrNull(raw.movedStopLoss),
    closedEarly: toBoolOrNull(raw.closedEarly),
    revengeTraded: toBoolOrNull(raw.revengeTraded),
    overLeveraged: toBoolOrNull(raw.overLeveraged),
    followedStrategy: toBoolOrNull(raw.followedStrategy)
  };
}
...
// FIX: the previous default (300000ms = 5 min) used 5 credits/poll × 288 polls/day
// = 1,440 Twelve Data credits/day — nearly double the free plan's 800/day cap,
// meaning the app could silently start failing mid-day. 600000ms (10 min) = 144
// polls/day × 5 credits = 720/day, safely under the cap with headroom.
const POLL_INTERVAL_MS = parseInt(process.env.MARKET_POLL_INTERVAL_MS, 10) || 600000;
...
function normalizePairKey(raw) {
  ...
  if (k === 'BTC') k = 'BTCUSD';
  if (k === 'ETH') k = 'ETHUSD';
  // FIX: previously only checked TRACKED_SYMBOLS (the core 5), so Running
  // trades on any of the 13 extended instruments (EURUSD, GBPJPY, etc.) were
  // silently skipped by monitorRunningTrades forever — they'd stay "Running"
  // no matter what the market did. EXTENDED_SYMBOLS is declared further down
  // this file but is safe to reference here: this function is only ever
  // invoked from async callbacks that run after the whole module has
  // finished its initial synchronous evaluation, by which point every
  // top-level const in this file — including EXTENDED_SYMBOLS — exists.
  if (TRACKED_SYMBOLS[k] || (typeof EXTENDED_SYMBOLS !== 'undefined' && EXTENDED_SYMBOLS[k])) return k;
  return null;
}

// Checks the core cache first (fresher, polled more often), then falls back
// to the extended cache. Returns null (never a guess) if neither has a price.
function getCachedPrice(key) {
  if (marketCache[key] && marketCache[key].price !== null && marketCache[key].price !== undefined) {
    return marketCache[key].price;
  }
  if (typeof extendedMarketCache !== 'undefined' && extendedMarketCache[key] && extendedMarketCache[key].price !== null && extendedMarketCache[key].price !== undefined) {
    return extendedMarketCache[key].price;
  }
  return null;
}
...
// Computes the R-multiple actually achieved on a closed trade using the
// trade's own entry/stopLoss as the risk unit (works for any closed trade,
// not just prop-firm-linked ones — independent of position size or $ risk).
function computeRealizedR(direction, entry, stopLoss, exit) {
  if (entry === undefined || entry === null || stopLoss === undefined || stopLoss === null || exit === undefined || exit === null) return null;
  entry = parseFloat(entry); stopLoss = parseFloat(stopLoss); exit = parseFloat(exit);
  if (isNaN(entry) || isNaN(stopLoss) || isNaN(exit)) return null;
  const riskUnit = direction === 'Buy' ? (entry - stopLoss) : (stopLoss - entry);
  if (riskUnit <= 0) return null; // misconfigured SL (wrong side of entry) — can't derive a meaningful R
  const move = direction === 'Buy' ? (exit - entry) : (entry - exit);
  return move / riskUnit;
}
...
async function monitorRunningTrades() {
  try {
    ...
    for (const trade of runningTrades) {
      ...
      if (!key) continue; // unsupported symbol — never guess
      const current = getCachedPrice(key);
      if (current === null) continue; // no live data yet for this symbol — leave OPEN, never guess
      ...
      if (!hitOutcome) {
        // Still running — just refresh the display price. Idempotent by nature
        // (re-writing the same live value), no atomicity concern here.
        await Trade.updateOne(
          { _id: trade._id, outcome: 'Running' },
          { $set: { currentPrice: current, lastPriceUpdate: new Date(), marketSymbol: key, priceSource: 'Twelve Data' } }
        );
        continue;
      }

      // CLOSING a trade: this is the operation that must be idempotent even if
      // two monitoring passes somehow detect the same trigger back-to-back.
      // The { outcome: 'Running' } filter guarantees the update only applies
      // once — if another pass already closed it, this becomes a safe no-op
      // (matchedCount 0) instead of double-applying P/L or flipping the result.
      const realizedR = computeRealizedR(dir, trade.entry, trade.stopLoss, current);
      const closeUpdate = {
        outcome: hitOutcome,
        exit: current,
        exitReason: hitReason,
        closedAt: new Date(),
      ...
      // Estimate realized $P/L for prop-firm-linked trades that haven't had one
      // manually entered — same R-multiple method as before, now reusing the
      // universal realizedR computed above for consistency.
      if (trade.propFirmAccountId && trade.riskAmount && (trade.realizedPL === null || trade.realizedPL === undefined)) {
        closeUpdate.realizedPL = realizedR !== null ? trade.riskAmount * realizedR : (hitOutcome === 'Win' ? trade.riskAmount : -trade.riskAmount);
      }
      ...
      if (result) {
        console.log(`Auto-closed trade ${result._id} (${result.pair} ${result.direction}) as ${result.outcome} via ${result.exitReason} at ${current}`);
      }
    }
  } catch (err) {
...
// ============================================================
// EXTENDED MARKETS (Phase 1 — Markets page)
// ============================================================
// This is a SEPARATE, ADDITIVE subsystem — it does not touch marketCache,
// fetchMarketData, or monitorRunningTrades above, which remain exactly as
// they were (dashboard mini-strip + TP/SL monitoring keep working unchanged).
//
// RATE-LIMIT REALITY CHECK (Twelve Data free plan: 8 credits/min, 800/day):
// The original 5-symbol feed already uses up to 720 credits/day at its new
// 10-minute interval. That leaves very little daily budget for 13 more
// symbols. Each full refresh of the 13 extended symbols costs 13 credits
// (batched in groups of ≤8 to respect the 8-credits/minute cap — a single
// 13-symbol call would exceed that limit in one shot). To stay within the
// remaining daily budget, the default extended refresh interval is 4 hours
// (13 credits × 6 refreshes/day = 78 credits/day). This is a genuine
// constraint of the free tier, not a design shortcut — raise
// EXTENDED_MARKET_POLL_INTERVAL_MS (and/or the original MARKET_POLL_INTERVAL_MS)
// if you upgrade your Twelve Data plan and want fresher extended-market data.
const EXTENDED_SYMBOLS = {
  EURUSD: { td: 'EUR/USD', label: 'EURUSD' },
  USDCHF: { td: 'USD/CHF', label: 'USDCHF' },
  AUDUSD: { td: 'AUD/USD', label: 'AUDUSD' },
  NZDUSD: { td: 'NZD/USD', label: 'NZDUSD' },
  EURGBP: { td: 'EUR/GBP', label: 'EURGBP' },
  EURJPY: { td: 'EUR/JPY', label: 'EURJPY' },
  GBPJPY: { td: 'GBP/JPY', label: 'GBPJPY' },
  GBPCHF: { td: 'GBP/CHF', label: 'GBPCHF' },
  AUDJPY: { td: 'AUD/JPY', label: 'AUDJPY' },
  EURAUD: { td: 'EUR/AUD', label: 'EURAUD' },
  EURCAD: { td: 'EUR/CAD', label: 'EURCAD' },
  GBPAUD: { td: 'GBP/AUD', label: 'GBPAUD' },
  ETHUSD: { td: 'ETH/USD', label: 'ETHUSD' }
};

const extendedMarketCache = {};
let extendedFailCount = 0;
const EXTENDED_POLL_INTERVAL_MS = parseInt(process.env.EXTENDED_MARKET_POLL_INTERVAL_MS, 10) || 14400000; // 4 hours

function chunkSymbols(map, size) {
...
async function fetchExtendedBatch(entries) {
  if (!process.env.TWELVE_DATA_API_KEY || !entries.length) return;
  const symbolsParam = entries.map(([, s]) => s.td).join(',');
  const url = `https://api.twelvedata.com/quote?symbol=${encodeURIComponent(symbolsParam)}&apikey=${process.env.TWELVE_DATA_API_KEY}`;
  try {
    const res = await fetch(url);
    const data = await res.json();
    const responseEntries = data.symbol ? { [data.symbol]: data } : data;
    let anySuccess = false;
    for (const [key, sym] of entries) {
      const q = responseEntries[sym.td];
      if (q && q.status !== 'error' && q.close !== undefined) {
        extendedMarketCache[key] = {
          symbol: sym.label,
          price: parseFloat(q.close),
          change: q.change !== undefined ? parseFloat(q.change) : null,
          percentChange: q.percent_change !== undefined ? parseFloat(q.percent_change) : null,
        ...
        anySuccess = true;
      } else if (extendedMarketCache[key]) {
        extendedMarketCache[key].status = 'stale';
      }
    }
    if (anySuccess) extendedFailCount = 0;
  } catch (err) {
    extendedFailCount++;
    console.error('Extended market fetch failed:', err.message);
    for (const [key] of entries) {
      if (extendedMarketCache[key]) extendedMarketCache[key].status = extendedFailCount >= 3 ? 'offline' : 'stale';
    }
  }
}
...
async function fetchExtendedMarketData() {
  if (!process.env.TWELVE_DATA_API_KEY) {
    for (const key of Object.keys(EXTENDED_SYMBOLS)) {
      extendedMarketCache[key] = extendedMarketCache[key] || { symbol: EXTENDED_SYMBOLS[key].label, price: null, change: null, percentChange: null, updatedAt: null, status: 'offline' };
    }
    return;
  }
  // Batches of ≤8 symbols, spaced 65s apart so the combined per-minute credit
  // usage never exceeds Twelve Data's 8-credits/minute free-tier cap.
  // FIX: previously scheduled with fire-and-forget setTimeout and never
  // triggered monitoring afterward, so trades on extended-instrument pairs
  // could sit unevaluated against a stale price for up to another full core
  // poll cycle after new data arrived. Now awaits each batch in sequence and
  // runs monitorRunningTrades() once immediately after the last one lands.
  const batches = chunkSymbols(EXTENDED_SYMBOLS, 7);
  for (let i = 0; i < batches.length; i++) {
    if (i > 0) await new Promise(resolve => setTimeout(resolve, 65000));
    await fetchExtendedBatch(batches[i]);
  }
  await monitorRunningTrades();
}

fetchExtendedMarketData();
setInterval(fetchExtendedMarketData, EXTENDED_POLL_INTERVAL_MS);

app.get('/api/market/extended', (req, res) => {
  res.json({
    success: true,
    pollIntervalMs: EXTENDED_POLL_INTERVAL_MS,
    configured: !!process.env.TWELVE_DATA_API_KEY,
    data: extendedMarketCache
  });
});
...
// Confirmed real response fields (from FMP's own published documentation):
//   event, date ("YYYY-MM-DD HH:MM:SS"), country, actual, previous, change,
//   changePercentage, estimate, impact ("Low"|"Medium"|"High").
// This is a genuinely documented, real macro-economic calendar (CPI, NFP,
// central bank rate decisions, GDP, etc.) — not scraped, not fabricated.
// FMP's free "Basic" plan grants 250 requests/day across most endpoints;
// whether this specific endpoint is included on Basic can change over time,
// so failures here are handled the same way as every other external
// provider in this app: keep the last good cache, mark it stale, never
// invent an event.
//
// Country -> currency mapping. FMP's `country` field uses mixed formats
// across sources (ISO codes and full names depending on the release), so
// both are matched. Anything not in this map is treated as "not one of the
// 8 currencies this app tracks" and filtered out of the default view rather
// than guessed at.
const COUNTRY_TO_CURRENCY = {
  US: 'USD', 'United States': 'USD',
  GB: 'GBP', UK: 'GBP', 'United Kingdom': 'GBP',
  EU: 'EUR', EMU: 'EUR', 'Euro Area': 'EUR', EA: 'EUR',
...
let economicCalendarStatus = 'offline'; // 'live' | 'stale' | 'offline'
let economicCalendarLastUpdated = null;
let economicCalendarFailCount = 0;
const ECONOMIC_CALENDAR_POLL_INTERVAL_MS = parseInt(process.env.ECONOMIC_CALENDAR_POLL_INTERVAL_MS, 10) || 1800000; // 30 min

function formatDateYMD(d) {
  return d.toISOString().slice(0, 10);
}

// Pure, testable normalization of a raw FMP economic-calendar response array
// into this app's internal event shape. Kept separate from the network call
// itself so it can be exercised directly by automated tests without needing
// a live API connection.
function normalizeEconomicEvents(rawArray) {
  return rawArray
    .filter(e => e && e.event && e.date)
    .map(e => ({
      event: sanitizeText(e.event, 200),
      date: e.date, // "YYYY-MM-DD HH:MM:SS", provider's own timestamp — displayed as-is, not reinterpreted into a guessed timezone
      country: e.country || null,
      currency: COUNTRY_TO_CURRENCY[e.country] || null,
      impact: e.impact || null,
      actual: (e.actual !== undefined && e.actual !== null) ? e.actual : null,
      forecast: (e.estimate !== undefined && e.estimate !== null) ? e.estimate : null,
      previous: (e.previous !== undefined && e.previous !== null) ? e.previous : null,
      change: (e.change !== undefined && e.change !== null) ? e.change : null,
      changePercentage: (e.changePercentage !== undefined && e.changePercentage !== null) ? e.changePercentage : null
    }))
...
async function fetchEconomicCalendar() {
  if (!process.env.FMP_API_KEY) {
    economicCalendarStatus = 'offline';
    return;
  }

  const from = formatDateYMD(new Date(Date.now() - 3 * 86400000));   // 3 days back
  const to = formatDateYMD(new Date(Date.now() + 21 * 86400000));    // 21 days ahead (within FMP's documented 90-day max window)
  const url = `https://financialmodelingprep.com/stable/economic-calendar?from=${from}&to=${to}&apikey=${process.env.FMP_API_KEY}`;
  ...
  try {
    ...
    if (!Array.isArray(data)) {
      // FMP returns an error object (e.g. {"Error Message": "..."}) rather
      // than an array when the key is invalid, the plan doesn't include this
      // endpoint, or the rate limit is hit. Never treat that as "no events".
      throw new Error((data && (data['Error Message'] || data.message)) || 'Unexpected response shape from economic calendar provider');
    }

    economicCalendarCache = normalizeEconomicEvents(data);

    economicCalendarStatus = 'live';
    economicCalendarLastUpdated = new Date();
    economicCalendarFailCount = 0;
  } catch (err) {
    economicCalendarFailCount++;
    console.error('Economic calendar fetch failed:', err.message);
    // Keep whatever was cached before — never wipe real data to empty just
    // because of a transient failure, and never fabricate a replacement.
    economicCalendarStatus = economicCalendarFailCount >= 3 ? 'offline' : 'stale';
  }
}

fetchEconomicCalendar();
setInterval(fetchEconomicCalendar, ECONOMIC_CALENDAR_POLL_INTERVAL_MS);
...
app.get('/api/economic-calendar', (req, res) => {
  res.json({
    ...
    lastUpdated: economicCalendarLastUpdated,
    pollIntervalMs: ECONOMIC_CALENDAR_POLL_INTERVAL_MS,
    events: economicCalendarCache
  });
});
...
app.post('/api/analyze', requireAuth, async (req, res) => {
  ...
  try {
    const {
      ...
      propFirmAccountId, riskAmount, realizedPL, psychology
    } = req.body;
    ...
    // Duplicate-submission guard: if this exact trade (same user/pair/direction/
    // entry/SL/TP) was already saved in the last 15 seconds, return that one
    // instead of creating a second row. This covers double-taps and network
    // retries without needing a client-supplied idempotency key.
    const dupWindowStart = new Date(Date.now() - 15000);
    const parsedEntry = entryPrice !== '' ? parseFloat(entryPrice) : undefined;
    const parsedSL = stopLoss !== '' && stopLoss !== undefined ? parseFloat(stopLoss) : undefined;
    const parsedTP = takeProfit !== '' && takeProfit !== undefined ? parseFloat(takeProfit) : undefined;
    const existingDup = await Trade.findOne({
      userId: req.user._id,
      pair: currencyPair,
    ...
    if (existingDup) {
      return res.json({
        success: true,
        text: (existingDup.aiStructured && existingDup.aiStructured.summary) || existingDup.aiAnalysisText || 'This trade was already analyzed a moment ago — showing the saved result instead of creating a duplicate.',
        structured: existingDup.aiStructured || null,
        trade: existingDup
      });
    }
    ...
    savedTrade = await Trade.create({
      ...
      psychology: parsePsychology(psychology),
  ...
  try {
    ...
    const prompt = `You are an elite forex/crypto trading mentor. Analyze this trade using ONLY the information given — do not invent details, do not assume information that wasn't provided, and do not claim certainty about outcomes.
...
{
  "verdict": one of "Strong Setup" | "Good Setup" | "Neutral Setup" | "Weak Setup" | "Avoid",
  "scores": { "technical": 0-100, "risk": 0-100, "execution": 0-100, "psychology": 0-100, "overall": 0-100 },
  ...
  "takeProfitQuality": "string",
  "rrQuality": "string",
  "executionMistakes": "string",
  "recommendedImprovement": "string"
}

Scoring rules: "technical" and "execution" and "riskManagement" should reflect what the entry/SL/TP/R:R actually show. "psychology" can only be assessed from the trader's own notes/thesis — if the notes don't reveal emotional state or decision-making process, set psychology to 50 and say so explicitly in its related text rather than inventing behavioral claims. "overall" should be a reasonable weighted average, not just copied from one category.`;

    const model = genAI.getGenerativeModel({ model: 'gemini-3.5-flash' });
    const result = await model.generateContent(prompt);
    const response = await result.response;
    const rawText = response.text();
    savedTrade.aiAnalysisText = rawText;

    // Try to parse the structured JSON. If Gemini wrapped it in markdown
    // fences, strip those first. If parsing fails for any reason, this is
    // NOT an error — we simply fall back to the raw text (old Phase 1
    // behavior), so a formatting hiccup never breaks trade saving or the
    // AI Mentor page.
    let structured = null;
    try {
      ...
      console.log('AI response was not structured JSON — falling back to plain text display for this trade.');
    }

    await savedTrade.save();
    res.json({
      success: true,
      text: structured ? structured.summary : rawText,
      structured,
      trade: savedTrade
    });
  } catch (error) {
