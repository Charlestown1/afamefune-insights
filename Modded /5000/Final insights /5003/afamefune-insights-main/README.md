...
- **Mobile nav fix (round 2):** the overflow bug wasn't fully fixed by round 1's truncation/`overflow-x: hidden` on `body`/`.app-shell` alone — `.app-main` had `overflow-y: auto` without a paired `overflow-x`, and per the CSS spec that silently makes the browser treat `overflow-x` as `auto` too, letting `.app-main` scroll sideways independently of its parents' safeguards. Fixed by explicitly setting `overflow-x: hidden` everywhere an `overflow-y` rule existed (`.app-main`, `.sidebar`, `.modal-content`, `.mobile-drawer`, `.view-container`), and bumped the service worker's cache version so this fix isn't masked by a stale cached copy on your phone.
- **Dashboard header restored:** the Dashboard view now leads with the full branded header (logo, title, tagline, contact links) and a "Welcome, [name]" card with inline Admin/Log out buttons, matching the original single-page design — while the hamburger/sidebar still handles navigation to every other page. The two designs are merged, not one replacing the other.

## Known limitations / what's next

- **Dashboard layout note:** the homepage is intentionally kept to just account stats, live core-market prices, the Quick Analyze AI form, and recent trade history. The Open Risk guardrail now lives on the Risk Manager page, and the Forex Session tracker now lives on the Markets page — both keep working exactly as before (same IDs, same update logic), just relocated to where they're contextually relevant instead of crowding the homepage.
- **Mobile nav fix:** an earlier build could overflow horizontally on some phones if the logged-in user's email/name was long enough to push the top bar wider than the screen, which visually looked like a broken half-blank layout. Fixed by truncating that text with ellipsis and adding `overflow-x: hidden` safeguards at the shell level; the mobile hamburger button and top bar were also shrunk down for a more compact, professional look, and the drawer now has an explicit ✕ close button (in addition to tapping the overlay or pressing Escape).
