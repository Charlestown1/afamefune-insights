...
function showDashboard() {
  ...
  [adminTabBtn, adminTabBtnMobile].forEach(btn => {
    if (!btn) return;
...
function renderDashboardEconCalWidget() {
  const panel = document.getElementById('dashboard-econcal-panel');
  const widget = document.getElementById('dashboard-econcal-widget');
  if (!panel || !widget) return;

  if (!econCalConfigured) { panel.style.display = 'none'; return; }

  const now = new Date();
  const upcoming = latestEconomicEvents
  ...
  const msAway = new Date(next.date) - now;
  widget.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center;">
      <div>
        <div style="font-weight:700; font-size:0.88rem;">${escapeHtml(next.event)}</div>
        <div style="font-size:0.75rem; color:var(--text-dim); margin-top:2px;">${next.currency} · ${new Date(next.date).toLocaleString(undefined, { weekday: 'short', hour: '2-digit', minute: '2-digit' })}</div>
      </div>
      <span class="trade-outcome-tag" style="color:var(--loss); background:transparent; border:1px solid var(--loss);">HIGH</span>
    </div>
    <div class="session-sub-line" style="margin-top:8px;">In ${formatHMS(msAway)}</div>
  `;
}
